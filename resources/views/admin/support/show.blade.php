...
<style>
    ...
    /* Giữ phần hội thoại cuộn riêng, phần trả lời luôn hiển thị */
.chat-container{
  display:flex;
  flex-direction:column;
  /* Chiều cao linh hoạt theo viewport, không bị cắt */
  max-height: calc(100vh - 280px);
  overflow: hidden; /* giữ border radius đẹp, không tràn */
}
.chat-area{
  flex: 1 1 auto;
  overflow-y: auto;   /* chỉ khối tin nhắn cuộn */
}
.reply-section{
  flex: 0 0 auto;     /* luôn nằm trong khung, không bị co/cắt */
}

/* Nếu màn nhỏ hơn, hạ chiều cao một chút */
@media (max-width: 992px){
  .chat-container{ max-height: calc(100vh - 220px); }
}
/* Card trái cao theo viewport -> bên trong mới có chỗ để cuộn */
.glass-card{
  display:flex;
  flex-direction:column;
  height: calc(100vh - -150px);   /* chỉnh 140px theo header/topbar của bạn */
}

/* Container chat co giãn trong card, KHÔNG còn max-height cố định */
.chat-container{
  flex: 1 1 0;
  display:flex;
  flex-direction:column;
  min-height: 0;                 /* QUAN TRỌNG: cho phép con co lại */
  overflow: hidden;
  max-height: none !important;   /* xoá giới hạn 600px cũ */
}

/* Chỉ vùng tin nhắn cuộn (scrollbar nội bộ) */
.chat-area{
  flex: 1 1 0;
  min-height: 0;                 /* QUAN TRỌNG: để có scrollbar */
  overflow-y: auto;
  max-height: none !important;   /* xoá giới hạn 500px cũ */
  -webkit-overflow-scrolling: touch;
  overscroll-behavior: contain;  /* tránh “cuộn lan” sang body */
}

/* Vùng nhập ở dưới, không co */
.reply-section{ flex: 0 0 auto; }
            ...
            <div class="glass-card">
                ...
                <div class="chat-container">
                    <div class="chat-area" id="msgBox">
                        ...
                        @php
                        // Admin đang xem → tin của staff là "của mình"
                        $mine = (bool) $m->is_staff;
                        $sender = $m->user->fullname ?? ($mine ? 'Nhân viên' : 'Khách hàng');
                        $bg = $mine ? 'linear-gradient(135deg,#6E8EFB,#A777E3)' : 'rgba(255,255,255,0.92)';
                        $color = $mine ? '#fff' : '#111827';
                        $meta = $mine ? 'rgba(255,255,255,.85)' : '#667085';
                        @endphp

                        <div style="display:flex; margin:12px 0; justify-content: {{ $mine ? 'flex-end' : 'flex-start' }};">
                            <div style="
        max-width:70%; padding:14px 16px; border-radius:20px;
        box-shadow:0 10px 24px rgba(0,0,0,.10); backdrop-filter: blur(6px);
        background: {{ $bg }}; color: {{ $color }};
      ">
                                <div style="display:flex; justify-content:space-between; gap:12px; margin-bottom:8px; font-size:.88rem; color: {{ $meta }};">
                                    <div>{!! $mine ? '🛡️' : '👤' !!} {{ $sender }}</div>
                                    <div>{{ $m->created_at->format('d/m/Y H:i') }}</div>
                                </div>

                                <div style="line-height:1.6;">{!! nl2br(e($m->body)) !!}</div>

                                @if ($m->attachments && $m->attachments->count())
                                <div style="display:flex; flex-wrap:wrap; gap:12px; margin-top:10px;">
                                    @foreach ($m->attachments as $att)
                                    @php
                                    $url = $att->url ?? \Illuminate\Support\Facades\Storage::url($att->path);
                                    $isImg = \Illuminate\Support\Str::startsWith($att->mime ?? '', 'image/');
                                    @endphp
                 ...
                 ">
                   ...
                   {{ $mine ? 'background:rgba(255,255,255,.18); color:#fff;' : 'background:#fff; color:#111827;' }}
                 ">
                                        📄 {{ $att->original_name }}
                                    </a>
                                    @endif
                        ...
                        @empty
                        <div style="text-align:center; color:rgba(0,0,0,.5); padding:3rem;">
