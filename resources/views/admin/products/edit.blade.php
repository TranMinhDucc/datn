...
<script>
  function removeSizeChart() {
    document.getElementById('size_chart_box')?.remove();
    document.getElementById('remove_size_chart').value = '1';
  }

  const inputSC = document.getElementById('size-chart-input');
  inputSC.addEventListener('change', function (e) {
    const files = e.target.files;
    if (!files || files.length === 0) return;

    // ✅ Nếu user chọn >1 file (do kéo thả…), chỉ lấy file đầu
    if (files.length > 1) {
      const dt = new DataTransfer();
      dt.items.add(files[0]);
      inputSC.files = dt.files;
    }

    // ✅ Khi chọn ảnh mới → coi như thay thế ảnh cũ
    removeSizeChart();

    // ✅ Preview đúng 1 ảnh
    const c = document.getElementById('size-chart-preview-container');
    c.innerHTML = '';
    const f = inputSC.files[0];
    ...
    r.onload = ev => {
      ...
      btn.innerHTML = '&times;';
      btn.onclick = () => { wrap.remove(); inputSC.value = ''; };
