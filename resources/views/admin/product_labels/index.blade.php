                        ...
                        <tbody class="fw-semibold text-gray-600">
                            @foreach ($labels as $index => $label)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>
                                        <img src="{{ asset($label->image) }}" alt="label" width="60" class="rounded shadow-sm">
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-end">
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end">
                                                <li>
                                                    <a href="{{ route('admin.product-labels.edit', $label->id) }}" class="dropdown-item">
                                                        <i class="fa fa-edit me-2"></i> Sửa
                                                    </a>
                                                </li>
                                                <li>
                                                    <form action="{{ route('admin.product-labels.destroy', $label->id) }}" method="POST" onsubmit="return confirm('Xoá nhãn này?')">
                                                        @csrf @method('DELETE')
