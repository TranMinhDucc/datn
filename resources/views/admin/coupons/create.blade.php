                ...
                <form action="{{ route('admin.coupons.store') }}" method="POST">
                    ...
                    <div class="row mb-6">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="type" class="form-select form-select-solid" required>
                                <option value="product_discount" {{ old('type') == 'product_discount' ? 'selected' : '' }}>Sản phẩm</option>
                                <option value="shipping_discount" {{ old('type') == 'shipping_discount' ? 'selected' : '' }}>Phí vận chuyển</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row mb-6">
                        <div class="col-md-6" id="maxDiscountWrapper"
                            style="{{ old('value_type','percentage') === 'percentage' ? '' : 'display:none' }}">
                            ...
                            <input type="number" step="0.01" name="max_discount_amount" id="maxDiscountInput"
                                ...
                                {{ old('value_type','percentage') === 'percentage' ? '' : 'disabled' }}>
                        ...
                        <div class="col-md-4">
                            <label class="form-check form-switch">
                                <input type="checkbox" class="form-check-input" name="only_for_new_users"
                                    value="1" {{ old('only_for_new_users') ? 'checked' : '' }}>
                    ...
                    <div class="form-check form-switch mb-4">
                        <input class="form-check-input" type="checkbox" id="applyAllProducts" name="apply_all_products"
                            value="1" {{ old('apply_all_products', 1) ? 'checked' : '' }}>
                        <label class="form-check-label fw-semibold" for="applyAllProducts">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="applicable_product_ids" id="productIdsInput"
                            class="form-control form-control-solid"
                            value="{{ old('applicable_product_ids') }}"
                            placeholder="VD: 1,2,3" {{ old('apply_all_products', 1) ? 'disabled' : '' }}>
                    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const select = document.getElementById('valueTypeSelect');
        const wrap = document.getElementById('maxDiscountWrapper');
        const input = document.getElementById('maxDiscountInput');

        function toggleMaxDiscount() {
            const isPercent = select.value === 'percentage';
            wrap.style.display = isPercent ? '' : 'none';
            input.disabled = !isPercent;
            if (!isPercent) input.value = ''; // tránh gửi giá trị cũ lên server
        }

        select.addEventListener('change', toggleMaxDiscount);
        toggleMaxDiscount(); // chạy lần đầu
    });
</script>
