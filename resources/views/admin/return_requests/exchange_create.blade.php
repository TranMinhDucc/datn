...
@section('content')
    <div class="container-fluid">
        ...
        <form action="{{ route('admin.return-requests.exchange.create', ['id' => $returnRequest->id]) }}" method="POST">
            @csrf
                            ...
                            <div class="item-row row mb-3 align-items-end">
                                    ...
                                    <select name="items[{{ $index }}][variant_id]"
                                        ...
                                        @foreach ($product->variants as $v)
                                            <option value="{{ $v->id }}"
                                                ...
                                                {{ $v->variant_name }} ({{ number_format($v->price, 0) }} VNĐ - Tồn kho:
                                                {{ $v->quantity ?? 0 }})
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group col-md-2">
                                    ...
                                    <input type="number" name="items[{{ $index }}][quantity]" class="form-control"
                                        min="1" value="{{ $item->orderItem->quantity }}" required>
                                </div>
