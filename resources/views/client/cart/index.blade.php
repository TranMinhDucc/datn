    ...
    <div class="custom-container container">
        <div class="row g-4">
            ...
            <div class="col-xxl-3 col-xl-4">
                ...
                <a href="{{ route('client.account.checkout') }}" id="checkout-btn" class="btn btn_black w-100 rounded sm">Check Out</a>
    ...
    try {
        ...
        if (invalidItems.length > 0) {
            invalidItems.forEach(item => {
                const row = document.querySelector(`[data-cart-id="${item.id}"][data-variant-id="${item.variant_id ?? ''}"]`);
                if (row) {
                ...
                showToast(item.message || `Sản phẩm "${item.name}" hết hàng`, "warning");
            });
