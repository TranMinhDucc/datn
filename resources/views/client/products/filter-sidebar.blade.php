                ...
                <div class="custom-accordion theme-scrollbar left-box">
                    ...
                    <form method="GET" action="{{ route('client.products.filterSidebar') }}">
                        <div class="accordion" id="accordionPanelsStayOpenExample">
                            ...
                            <div class="accordion-item">
                                <h2 class="accordion-header">
                                    <button class="accordion-button" data-bs-toggle="collapse" data-bs-target="#collapseTag">
                                        <span>Tag</span>
                                    </button>
                                </h2>
                                ...
                                <div id="collapseTag" class="accordion-collapse collapse show">
                                    <div class="accordion-body">
                                        {{-- Chế độ ghép điều kiện: any/all --}}
                                        <div class="d-flex align-items-center justify-content-between mb-2">
                                            <label class="me-2 small text-muted">Ghép điều kiện</label>
                                            <select name="tags_mode" class="form-select form-select-sm w-auto">
                                                <option value="any" {{ request('tags_mode','any')=='any' ? 'selected' : '' }}>Bất kỳ</option>
                                                <option value="all" {{ request('tags_mode')=='all' ? 'selected' : '' }}>Tất cả</option>
                                            </select>
                                        </div>

                                        <ul class="color-variant theme-scrollbar">
                                            @php $selectedTags = (array) request('tags', []); @endphp
                                            @foreach ($allTags as $tag)
                                            @php $checked = in_array($tag->slug, $selectedTags); @endphp
                                            <li>
                                                <input class="custom-checkbox" id="tag{{ $tag->id }}"
                                                    type="checkbox" name="tags[]" value="{{ $tag->slug }}"
                                                    {{ $checked ? 'checked' : '' }}>
